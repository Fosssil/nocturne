...
vim.api.nvim_set_hl(0, "HlSearchLensIcon", { link = "Comment" })
vim.api.nvim_set_hl(0, "HlSearchLensNear", { link = "Search", bold = true })
...
return {
	...
	config = function(_, opts)
		...
		local function search(key)
			return function()
				-- Prevent an unsuccessful search from throwing an ugly command-line error.
				pcall(vim.cmd.normal, { key, bang = true })
				hlslens.start()
			end
		end
		...
		vim.keymap.set("n", "*", search_from_top("*"), { desc = "Search Word Forward" })
		vim.keymap.set("n", "#", search_from_top("#"), { desc = "Search Word Backward" })
		vim.keymap.set("n", "g*", search_from_top("g*"), { desc = "Search Partial Word Forward" })
		vim.keymap.set("n", "g#", search_from_top("g#"), { desc = "Search Partial Word Backward" })
